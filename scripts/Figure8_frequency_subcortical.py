...
micro_df     = cort_df[cort_df['Electrode_type'] == 'Microelectrode']
...
cort_miss_labels = ['LPons','Amy','LVPal']
...
for label in reversed_brain_numbers:
    ...
    if label == 0:        
        ...
        if filtered_df['Frequency_Range'].notna().any():
            ...
            for value, count in zip(unique_values, counts):
                    ax4.scatter(int(value), y_counter, s=15*count, label=None, edgecolors='none', zorder=4, color='indianred')  # Set label=None to avoid duplicate labels
...
ax2.bar(singular_frequency_count.keys(), range_frequency_count, color='indianred', edgecolor='none', width=10, alpha=1, zorder=1)
...
ax2.set_ylim(0,150)  # Match y-limits
...
ax2.axvline(x=median_value, ymin=0, ymax=125, color='#d8dcd6', linestyle='--', linewidth=1)
ax2.scatter(median_value, 125, s=100, facecolors='white', edgecolors=single_frequency_colour, linewidths=5, zorder=5)

# Save output as a .png and .svg file
plt.tight_layout()

output_path = '../outputs/'
plt.savefig(output_path + 'Figure2B.svg', format='svg', dpi=600)
plt.savefig(output_path + 'Figure2B.png', format='png', dpi=600)

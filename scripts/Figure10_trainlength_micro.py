...
micro_single   = df_single[df_single['Electrode_type'] == 'Microelectrode']
micro_multiple = df_multiple[df_multiple['Electrode_type'] == 'Microelectrode']
micro_range    = df_range[df_range['Electrode_type'] == 'Microelectrode']
...
micro_single_size   = len(micro_single)
micro_multiple_size = len(micro_multiple)
micro_range_size    = len(micro_range)

micro_sizes    = [micro_single_size, micro_multiple_size, micro_range_size, micro_sp, micro_sec, micro_long, micro_unclear]

ax1.pie(micro_sizes, colors=colors, wedgeprops={'linewidth': 1, 'edgecolor': 'white'}, startangle=90)
ax1.axis('equal')
...
# --- Flatten single-frequency durations
single_values = micro_single['Timespan'].explode().astype(int)
value_counts  = single_values.value_counts()

# --- Flatten multiple-frequency durations
multiple_values = micro_multiple['Timespan'].str.split(',').explode().astype(int)
multiple_counts = multiple_values.value_counts()

# --- Create binary arrays for ranged values
micro_range['Binary_ms'] = micro_range['Timespan_Range'].apply(
    create_binary_array, start_value=0, end_value=60000
)
collapsed_array = np.sum(np.array(micro_range['Binary_ms'].tolist()), axis=0)
...
split_vals = micro_range['Timespan'].str.split('-', expand=True).apply(pd.to_numeric)
micro_range['Midpoint'] = split_vals.mean(axis=1)
micro_range['Interval'] = split_vals[1] - split_vals[0]

micro_midpoint_median = micro_range['Midpoint'].median()
micro_interval_median = micro_range['Interval'].median()
...
ax2.scatter(micro_midpoint_median, 140, s=100, facecolors='white',
            edgecolors=range_frequency_colour, linewidths=5, zorder=5)

ax2.axvline(x=micro_midpoint_median, color='#d8dcd6', linestyle='--', linewidth=1)

# Save output as a .png and .svg file
plt.tight_layout()

output_path = '../outputs/'
plt.savefig(output_path + 'Figure9A.svg', format='svg', dpi=600)
plt.savefig(output_path + 'Figure9A.png', format='png', dpi=600)

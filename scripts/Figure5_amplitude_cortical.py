...
" (5) Create a pie chart that shows the distribution of stimulation types - micro and macro-electrode"
# Add labels and title
sizes    = [len(micro_df), len(macro_df), len(df_voltage), len(df_unclear)]
colors   = ['indianred', 'darkorange', 'navajowhite', 'silver']
...
"(4) Bar graph with counts -- per amplitude"
...
df_amplitude.loc[:, 'BinaryArrays'] = df_amplitude['Range'].apply(
    create_binary_array, start_value=1, end_value=10000
)
...
cort_miss_labels = ['TG','V6/V6A','MCC']
...
for label in reversed_brain_numbers:
    ...
    if label == 0:        
        ...
        if filtered_df['Electrode_type'].str.contains('Microelectrode', case=False, na=False).any():
            ...
            #ax4.scatter(x, np.full_like(collapsed_array, y_counter), s=dot_sizes, color='indianred', alpha=1)
        ...
        if filtered_df['Electrode_type'].str.contains('Macroelectrode', case=False, na=False).any():
            ...
            # Plot each x as a dot on the line at y_target
            #ax4.scatter(x, np.full_like(collapsed_array, y_counter), s=dot_sizes, color='darkorange', alpha=1)
...
output_path = '../outputs/'
plt.savefig(output_path + 'Figure4_Cortical.svg', format='svg', dpi=600)
plt.savefig(output_path + 'Figure4_Cortical.png', format='png', dpi=600)

...
task_colors = {
    'eyes': '#1f77b4',          # blue - visual / eye movement
    'hands': '#66c266',         # light green - manual / motor actions
    'movement': '#2ca02c',      # darker green - general movement
    'neural': '#9467bd',        # purple - neural activity
    'hormonal': '#ff69b4',      # pink - endocrine / reward
    'hemodynamic': '#d62728',   # red - blood flow / vascular
}

task_categories = list(task_colors.keys())

# Effector-based task colors – smooth & blend-optimised
task_colors = {
    # Behavioural (smooth, less aggressive, still distinct)
    'eyes': '#4FA3C7',       # soft steel cyan
    'hands': '#D9A441',      # muted warm amber
    'movement': '#6FBF73',   # soft fresh green

    # Physiological (dark, clearly separated)
    'neural': '#3B0F70',     # deep indigo
    'hormonal': '#7A001E',   # dark crimson
    'hemodynamic': '#00304D' # deep blue-teal
}
...
def prepare_stacked_data(df_subset):
    ...
    for num in all_numbers['Brain_Parc_Numb']:
        ...
        counts = df_roi['Main_output'].value_counts()
...
nib.save(nib.Nifti1Image(cor_color_vol, cor_affine, cor_header), data_path + 'cortical_colored_effectors.nii.gz')
nib.save(nib.Nifti1Image(sub_color_vol, sub_affine, sub_header), data_path + 'subcortical_colored_effectors.nii.gz')
